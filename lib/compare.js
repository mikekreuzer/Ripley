...
var getComparison = function(newData, fileSys=fs, mom=moment, comp=compare, templateReadCB=(err, oldData) => {
  if (err) throw err;
  comp(newData, JSON.parse(oldData));
}) {
  ...
  fileSys.readFile(path.join('data', fileOneYearAgo), 'utf8', templateReadCB);
};

var template = function(data, fileSys=fs, mom=moment, stach=mustache, errorFn = (err) => {
  if (err) throw err;
}) {
  const fileName = path.join('out', mom().format('MM-YYYY') + '.md');
  fileSys.readFile(path.join('lib', 'template.mst'), 'utf8', (err, templateFile) => {
    if (err) throw err;
...
var compare = function(newData, oldData, plate=template) {
