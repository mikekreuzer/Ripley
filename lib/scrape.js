...
let saveJSONFile = function(data, outPath, fs, dateFunc, comparisonFunc, errorFn = (err) => {
  if (err) throw err;
}) {
  const dateTime = dateFunc(),
    fileName = path.join(__dirname, '..', 'data', dateTime.format('MM-YYYY') + '.json');
  let dataToSave = {
  ...
  comparisonFunc(dataToSave, outPath);
};

module.exports.getParallel = async function(outPath,
  langs = languages,
  ...
  try {
    ...
    data = addIndex(data);
    saveJSON(data, outPath, fileSys, moment, compare.getComparison);
  } catch (err) {
    console.error(err);
  }
};
